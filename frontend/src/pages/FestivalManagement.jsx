// frontend/src/components/admin/FestivalMenuManagement.jsx
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { FaUpload, FaTrash, FaImage, FaSpinner, FaCheckCircle } from 'react-icons/fa';
...
const FestivalMenuManagement = () => {
  ...
  const fetchFestivals = async () => {
    try {
      ...
      if (response.data.success) {
        console.log('Festivals loaded:', response.data.festivals);
  ...
  const handleUpload = async (e) => {
    ...
    // Check if festival already has 2 images
    const festival = festivals.find(f => f._id === selectedFestival);
    if (festival && festival.menuImages && festival.menuImages.length >= 2) {
      alert('Maximum 2 menu images allowed per festival. Please delete an existing image first.');
      return;
    }
                        ...
                        <div key={image._id} className="menu-image-card">
                          ...
                          <div className="menu-image-footer">
                            <div className="menu-image-info">
                              ...
                              <small>Order: {image.order + 1}</small>
                            </div>
                            <button
          ...
          {festivals.map((festival) => (
            ...
            >
              <div className="festival-overview-content">
                ...
                {festival.menuImages && festival.menuImages.length === 2 && (
